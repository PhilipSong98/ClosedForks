-- Fix ambiguous column reference in optimized functions
-- This addresses the PostgreSQL error: column reference "group_id" is ambiguous

-- 1. Drop and recreate get_user_feed_optimized with explicit table aliases
...
-- 2. Also fix the other optimized functions if they have similar issues
...
BEGIN
  ...
  restaurant_aggregates AS (
    ...
    SELECT 
      ...
      ARRAY(
        SELECT DISTINCT unnest(rr2.tags)
        FROM restaurant_reviews rr2
        WHERE rr2.restaurant_id = rr.restaurant_id 
          AND rr2.tags IS NOT NULL 
          AND array_length(rr2.tags, 1) > 0
      ) as aggregated_tags
...
-- 3. Fix get_user_liked_reviews_optimized function
...
COMMENT ON FUNCTION get_user_feed_optimized IS 'Single-query function for homepage feed, eliminates N+1 queries by returning all needed data denormalized. Fixed ambiguous column references.';
COMMENT ON FUNCTION get_restaurants_feed_optimized IS 'Optimized restaurants feed with server-side filtering and keyset pagination. Fixed ambiguous column references.';
COMMENT ON FUNCTION get_user_liked_reviews_optimized IS 'Single-query function for user liked reviews with full denormalized data. Fixed ambiguous column references.';