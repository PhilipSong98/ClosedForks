-- Initial schema setup for Restaurant Reviews application
-- This represents the complete database schema without the problematic admin policies
...
-- Users table (references auth.users for proper authentication)
CREATE TABLE users (
...
-- Basic RLS Policies (safe, non-recursive)
...
COMMENT ON TABLE restaurants IS 'Restaurant information with location and details';
COMMENT ON TABLE reviews IS 'Multi-dimensional restaurant reviews';
COMMENT ON TABLE invites IS 'Invitation system for private network access';
COMMENT ON TABLE reports IS 'Content moderation system';

-- Note: Admin policies intentionally excluded to prevent infinite recursion
